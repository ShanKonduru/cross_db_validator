...
class EnhancedMarkdownReportGenerator:
    ...
    def _generate_progress_bar(self, passed: int, failed: int, skipped: int, total: int, width: int = 50) -> str:
    ...
    def _generate_category_breakdown(self) -> str:
        ...
        for result in self.test_results:
            ...
            if category not in category_stats:
                category_stats[category] = {'passed': 0, 'failed': 0, 'skipped': 0, 'total': 0}
...
"""
        ...
        for category, stats in sorted(category_stats.items()):
            ...
            table += f"| **{category}** | {stats['passed']} | {stats['failed']} | {stats['skipped']} | {stats['total']} | {success_rate:.1f}% |\n"
    ...
    def _generate_sheet_summary(self) -> str:
        ...
        for sheet_name, tests in self.sheets_data.items():
            ...
            success_rate = (sheet_passed / sheet_total * 100) if sheet_total > 0 else 0
            
            summary += f"""
### 📋 {sheet_name}

**Summary:** {sheet_total} tests | ✅ {sheet_passed} passed | ❌ {sheet_failed} failed | ⏭️ {sheet_skipped} skipped | Success Rate: **{success_rate:.1f}%**
...
- **Execution Status:** {'🟢 HEALTHY' if passed_pct >= 80 else '🟡 ATTENTION NEEDED' if passed_pct >= 60 else '🔴 CRITICAL'}
...
"""
    ...
    def _get_best_category(self) -> str:
        ...
        return f"{best_category} ({best_rate:.1f}%)" if best_category != "None" else "None"
    ...
    def _get_failing_categories(self) -> str:
        ...
        for category, stats in category_stats.items():
            if stats['failed'] > 0:
                ...
                failing_categories.append(f"{category} ({stats['failed']} failures, {fail_rate:.1f}%)")
